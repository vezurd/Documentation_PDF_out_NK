...
from base.tables_columns import CODE, TAGS, VALUES, NUMBERS, TYPE_MARK, NAME, TAGS_2, VALUES_2, NUMBERS_2, TYPE_MARK_2, \
    NAME_2, DIFF_FLAG_TAG, DIFF_FLAG_VALUE, DIFF_FLAG_NAME, DIFF_FLAG_TYPE, DIFF_FLAG_SUMMARY
...
def multi_mto_compare(all_base_unique: List[List[RowStd]], path_out_dir):
    ...
    title_row = RowStd()
    # Собираем все имена баз
    mto_names = []
    for base in all_base_unique:
        mto_names.append(file_name_remove_junk(base[0].t_com.file_name))
    ...
    title_row.el[VALUES].value = (f"Кол-во по\n "
                                  f"{mto_names[0]} (после изменений)")
    title_row.el[VALUES_2].value = (f"Кол-во по\n "
                                    f"{mto_names[0]} (до изменений)")
    title_row.el[DIFF_FLAG_VALUE].value = f"Различия Кол-ва"

    base_compare.append(title_row)

    main_base = all_base_unique[0]
    # Запоминаем изначальное значение
    for row in main_base:
        row.el[VALUES_2].value = row.el[VALUES].value
    print_att_list_table(STDTable.to_list(main_base))
    ...
    for i in range(len(all_base_unique)):
        if i != 0:
            for row in all_base_unique[i]:
                ...
                if code == "":
                    # finding_row = RowStd.get_row_copy(row)
                    main_base.append(row)
                    continue
                #
                # Ищем в main_base сроку по коду code
                # Если не нашли - то добавляем
                main_base_row = RowStd.get_row_by_code(code, main_base)
                if main_base_row is None:
                    # finding_row = RowStd(row.t_com)
                    main_base.append(row)
                    row.el[VALUES].comment += f"Новая трока из {row.t_com.file_name}\n"
                    row.el[VALUES].set_color_safe(Color.soft_pink)
                    row.el[NAME].set_color_safe(Color.soft_pink)
                    row.el[CODE].set_color_safe(Color.soft_pink)
                    row.el[DIFF_FLAG_VALUE].value = 1
                    continue
                #
                # Если нашли - то вычитаем значение из
                # Заносим значения в нужные столбцы по mto_att_dict_list[i]
                if isinstance(main_base_row, RowStd):
                    try:
                        old_value = main_base_row.el[VALUES].value
                        if old_value == "":
                            old_value = 0
                        new_value = row.el[VALUES].value
                        main_base_row.el[VALUES].value = int(old_value) - int(row.el[VALUES].value)
                    except ValueError as e:
                        print(f"\t@{main_base_row.el[VALUES].value}@")
                        print(f"\t@{row.el[VALUES].value}@")
                        raise e
                    comment_text = f"{row.t_com.file_name} - {row.el[VALUES].value}\n"
                    main_base_row.el[VALUES].comment += comment_text
                #
                else:
                    print(f"\tERROR in TWO_MTO_COMPARE: <{code}> - code, <{main_base_row}> code_check")
    ...
    for row in main_base:
        ...
        #
    print_att_list_table(STDTable.to_list(main_base))
    return_file_path = check_color_out(main_base,
                                       column_out_dict,
                                       path_out_dir,
                                       f"{1}_vs_{len(mto_names)-1}шт_файла",
                                       excel_template_check_color=r"templates\out_template_MTO_compare.xlsx",
                                       row_index=0,
                                       comment_height=300)
...
def two_mto_compare(
    all_base_unique: List[List[RowStd]],
    path_out_dir,
    auto_open_dir: bool = True,
):
    base_compare = []  # Список в который будем добавлять строки для вывода
    #
    # Строка заголовка таблицы
    ...
    title_row.el[DIFF_FLAG_SUMMARY].value = "Различия (сводка)"
    base_compare.append(title_row)
    #
    #
    mto_att_dict_list = [
    ...
    for row in base_compare:
        ...
        #
        _v1_raw = row.el[VALUES].value
        _v2_raw = row.el[VALUES_2].value
        _v1 = _normalize_number(_v1_raw)
        _v2 = _normalize_number(_v2_raw)
        if _numbers_equal(_v1, _v2):
            row.el[VALUES].color = Color.green
            row.el[VALUES_2].color = Color.green
        else:
            # print(f"Не равны: [{str(_v1_raw)}] [{str(_v2_raw)}]")
            row.el[VALUES].color = Color.yellow
            row.el[VALUES_2].color = Color.yellow
            row.el[DIFF_FLAG_VALUE].value = 1
        ...
        #
        if (
            row.el[DIFF_FLAG_TAG].value
            or row.el[DIFF_FLAG_VALUE].value
            or row.el[DIFF_FLAG_NAME].value
            or row.el[DIFF_FLAG_TYPE].value
        ):
            row.el[DIFF_FLAG_SUMMARY].value = 1
        else:
            row.el[DIFF_FLAG_SUMMARY].value = ""
        #
        if row.el[CODE].value == "":
            row.el[CODE].color = Color.red
        #
    ...
    column_out_dict = {
        ...
        13: DIFF_FLAG_NAME,
        14: DIFF_FLAG_TYPE,
        15: DIFF_FLAG_SUMMARY,
    }
    return_file_path = check_color_out(base_compare,
                                       column_out_dict,
                                       path_out_dir,
                                       f"{mto1_name}_vs_{mto2_name}",
                                       excel_template_check_color=r"templates\out_template_MTO_compare.xlsx",
                                       row_index=0,
                                       comment_height=300)
    if auto_open_dir and return_file_path:
        open_dir(return_file_path)
    return base_compare
...
def _normalize_number(value):
    ...
    Пустые строки/None/пробелы трактуются как None (отсутствие значения).
    ...
    if s == "" or s.lower() in {"none", "nan", "null", "-", "—"}:
        return None
    # Заменяем запятую на точку и убираем пробелы-разделители тысяч
    s = s.replace("\u00A0", " ").replace(" ", "")  # неразрывный пробел и обычные пробелы
    s = s.replace(",", ".")
    try:
        return float(s)
    except ValueError:
...
def _numbers_equal(a, b, eps: float = 1e-9) -> bool:
    """
    Считает значения равными если:
    - оба отсутствуют (None)
    - оба числа и равны с заданной точностью
    """
    if a is None and b is None:
        return True
...
excel_template_RFQ_compare = r"templates\out_template_RFQ_compare.xlsx"
