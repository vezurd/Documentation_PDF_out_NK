"""Child-process entry for RD MTO dump scans. Does not import Qt."""
...
def _arguments(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="RD catalog RD-tree dump scan worker (MTO xlsx + OD doc)"
    )
    parser.add_argument(
        "--job",
        type=Path,
        required=True,
        help="JSON file with CatalogConfig paths and optional cancel_path",
    )
    return parser.parse_args(argv)


def main(argv: list[str] | None = None) -> int:
    """Load a job file, walk the RD dump, and stream JSON status lines.
    ...
    if not isinstance(payload, dict):
        ...
        return 2
    try:
        config = config_from_rd_dump_job(payload)
    ...
    def progress(item: RdDumpScanProgress) -> None:
        _emit(
            {
                "t": "progress",
                "path": item.path,
