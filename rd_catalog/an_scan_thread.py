"""Qt thread that pumps a child-process AN dump scan without holding the GIL."""
...
from rd_catalog.an_scan import AnScanProgress, an_job_from_config
from rd_catalog.config import CatalogConfig

_PROJECT_ROOT = Path(__file__).resolve().parents[1]


class AnScanThread(QThread):
    """Start ``python -m rd_catalog.an_scan_cli`` and forward JSON status lines."""
    ...
    def __init__(self, config: CatalogConfig, parent=None) -> None:
        """Store an immutable AN dump scan request.

        Args:
            config: Resolved catalog configuration (``an_root``, ``db_path``,
                ``runtime_dir``).
    ...
    def request_cancel(self) -> None:
        ...
        if self._cancel_path is not None:
            try:
                ...
                pass
        self.log.emit("Запрошена отмена скана АН. Завершаем текущую операцию…")

    def run(self) -> None:
        """Launch the AN scan worker process and pump its stdout until exit."""

        runtime_dir = Path(self._config.runtime_dir)
        runtime_dir.mkdir(parents=True, exist_ok=True)
        stamp = f"{os.getpid()}_{threading.get_ident()}"
        job_path = runtime_dir / f"an_scan_job_{stamp}.json"
        cancel_path = runtime_dir / f"an_scan_cancel_{stamp}.flag"
        self._cancel_path = cancel_path
        job_path.write_text(
            json.dumps(
                an_job_from_config(self._config, cancel_path=str(cancel_path)),
                ensure_ascii=False,
                indent=2,
            ),
            encoding="utf-8",
        ...
        try:
            ...
            if self.failure is None and exit_code not in (0, 1):
                ...
                self.failure = f"Процесс скана АН завершился: {detail}"
