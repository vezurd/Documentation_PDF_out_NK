...
class RfpPartsPanel(QWidget):
    ...
    def _build_actions_group(self, parent: QWidget) -> QGroupBox:
        ...
        self._add_action_with_comment(
            ...
            v,
            "Проверить реестр",
            "Один реестр в папке _RFP. Перед заменой текущий файл копируется "
            "в _old_Реестры УЛ ДС с датой и временем. Если файл открыт в Excel, "
            "копия с датой рядом не пишется: можно остановить проверку или "
            "продолжить с уже подготовленным реестром.",
        ...
        self._add_action_with_comment(
            ...
            "Таблица копируется в письмо и пишется в xlsx: "
            "RFP сводный файл\\_теги_позиций_RFP.",
