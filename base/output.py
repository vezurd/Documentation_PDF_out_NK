...
def output_get_std_from_file(t_com: TableComments):
    # 01.1 Ищем "СО.xlsx"
    mto_path = t_com.file_full_path
    file_path = t_com.file_full_path
    if t_com.file_full_path != "-1":
        if t_com.dir_path != "-1":
            ...
            for doc in curr_mto:
                ...
                if doc.file_name == "output.xlsx":
                    t_com.file_full_path = doc.file_full_path
    # 01.2 Читаем лист
    output_raw_obj = load_from_xlsx_file(t_com)
    # 01.3 Нормализуем данные (по столбцам, формату, лишним символам и т.п.)
    output_std = list_raw_to_std(output_raw_obj, t_com)
    return output_std
